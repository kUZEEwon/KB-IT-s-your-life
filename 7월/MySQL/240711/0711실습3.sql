...
SELECT TRUNCATE(avg(salary),0), max(salary), min(salary), sum(salary)
FROM employees
...
SELECT 
    ...
    AVG(commission_pct * salary) AS average_bonus,
...
SELECT 
    ...
    ROUND(AVG(salary),1) AS average_salary,             -- 부서별 평균 급여
...
FROM 
    employees
WHERE 
    job_id NOT LIKE CONCAT('SA_',"%")  -- 'SA_'로 시작하는 업무를 제외
GROUP BY 
    job_id
HAVING 
    SUM(salary) > 10000     -- 전체 월급이 10,000을 초과하는 경우
ORDER BY 
    합계 DESC;      -- 급여 합계로 내림차순 정렬
