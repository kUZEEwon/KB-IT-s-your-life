...
<script>
    ...
    async function people_get() {
        try {
            const response = await axios.get('https://kobis.or.kr/kobisopenapi/webservice/rest/people/searchPeopleList.json', {
                params: {
                    key: '4213aad30c4b16a57806a7c69ad920be',
                    targetDt: '20240622'
                }
            });
            ...
            for(const people of peopleList){
                peopleDetailsHtml +=
                    '<div><strong>영화인 코드: </strong>'+people.peopleCd+ '</div>' +
                    '<div><strong>영화인 명: </strong> ' + people.peopleNm + '</div>' +
                    '<div><strong>영화인명(영문): </strong> ' + people.peopleNmEn + '</div>' +
                    '<div><strong>분야: </strong> ' + people.repRoleNm + '</div>' +
                    '<div><strong>필모리스트: </strong> ' + people.filmoNames + '</div>' +
                    '<hr/>';
                console.log(people);

                // 서버로 데이터를 순차적으로 전송하기
                await sendPeopleToServer(people);
